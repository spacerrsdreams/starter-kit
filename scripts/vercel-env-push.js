...
function main() {
  ...
  const args =
    varIdx === -1
      ? rawArgs
      : [...rawArgs.slice(0, varIdx), ...rawArgs.slice(varIdx + 2)];

  if (args.length < 2) {
    console.error(
      "Usage: env:push <env-file> <environment> [environment ...] [--var VARIABLE_NAME]",
    );
    console.error("Example: env:push .env.development preview");
    console.error("Example: env:push .env.production production");
    console.error(
      "Example: env:push .env.production production --var DATABASE_URL",
    );
  ...
  for (const env of environments) {
    if (!VALID_ENVS.has(env)) {
      console.error(
        `Error: Invalid environment "${env}". Use: development | preview | production`,
      );
      process.exit(1);
    }
  }
  ...
  if (singleVar) {
    ...
    console.log(
      `Pushing 1 variable "${singleVar}" from ${fileArg} to: ${environments.join(", ")}\n`,
    );
  } else {
    if (vars.length === 0) {
    ...
    console.log(
      `Pushing ${vars.length} variable(s) from ${fileArg} to: ${environments.join(", ")}\n`,
    );
  }
